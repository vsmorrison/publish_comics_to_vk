...
def main():
    ...
    comics_meta = xkcd_utilities.get_xckd_comics_meta(
        random.randint(1, latest_comics)
    )
    comics_filepath = xkcd_utilities.save_xckd_pic_file(comics_meta)
    ...
    uploaded_photo = vk_utilities.upload_picture_to_server(
        upload_data['response']['upload_url'], comics_filepath)
